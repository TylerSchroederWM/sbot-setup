...
function fetchChannelMessagesFrom(client, channelName, trackedChannelMessages, hops) {
	...
	var messageStream = client.messagesByType({
		// gt: trackedChannelData.lastMsgTimestamp, // only fetch messages that aren't already in cache; the first time we query a channel, this is zero
		reverse: true,
		type: "post"
	})
	...
	pull(messageStream, pull.filter(function(msg) {
		// taskData.lastTimestamp = msg.timestamp; // abuse of filter to always get the most recent timestamp

		if(followedIds.includes(msg.value.author.toLowerCase())) {
			var hasMsgText = msg.value.content.text && typeof(msg.value.content.text) == "string" && msg.value.content.text.includes(channelName);
			var hasChannelTag = msg.value.content.channel && msg.value.content.channel === channelName.substring(1);
			return hasMsgText || hasChannelTag;
		}
		return false;
